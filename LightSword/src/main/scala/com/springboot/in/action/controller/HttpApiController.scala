 ...
 */
@RestController
@RequestMapping(Array("/httpapi")) //构造注入dao
class HttpApiController @Autowired() (val HttpSuiteDao: HttpSuiteDao, val HttpApiDao: HttpApiDao, val HttpReportDao: HttpReportDao) {
    ...
    //分页查询用例列表
    @RequestMapping(value  = { Array("/json") }, method = Array(RequestMethod.GET))
    def listJson() = HttpApiDao.findAll()

    //根据用例集id，分页查询对应下的用例
    @RequestMapping(value  = { Array("/listHttpSuiteTestCase") }, method = Array(RequestMethod.GET))
    ...
    //
    @RequestMapping(value  = { Array("/listHttpSuiteTestCaseJson") }, method = Array(RequestMethod.GET))
    @ResponseBody
    ...
    @ResponseBody
    def delHttpApiById(@RequestParam(value = "id") httpApiId: Integer): Boolean = {
        val re = HttpApiDao.deleteById(httpApiId)
        if (re != 0) {
            return true
        } else {
            return false
    ...
    //在新建用例页面,调试用例用
    @RequestMapping(value  = Array("/debugTest"), method = Array(RequestMethod.GET))
    @ResponseBody
    ...
    //执行用例
    @RequestMapping(value  = Array("/runTest"), method = Array(RequestMethod.GET))
    @ResponseBody
    def debugTest(@RequestParam(value = "id") id: Integer) = {
        runTestCase(id)
    }

    //回归项目全部用例,每个用例单独起一个线程跑，测试某个用例集下的所有用例
    @RequestMapping(value  = Array("/testHttpSuite"), method = Array(RequestMethod.GET))
    @ResponseBody
    def testProject(@RequestParam(value = "httpSuiteId") httpSuiteId: Integer) = {
        ...
        println("回归测试开始......")
        countDownLatch.await // 等待线程全部执行完
        println("回归测试结束!")
